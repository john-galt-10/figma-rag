...
def load_sentence_transformer(model_name: str):
    ...
    if local_model_path is not None and local_model_path.is_dir():
        print(
            f"Loading local embedding model for {model_reference}: {local_model_path}",
            file=sys.stderr,
        )
    ...
    if local_model_path is not None:
        print(
            f"Local embedding model path not found for {model_reference}: "
            f"{local_model_path}. Loading from Hugging Face/cache.",
            file=sys.stderr,
        )
    else:
        print(
            f"No local embedding model mapping found for {model_reference}. "
            "Loading from Hugging Face/cache.",
            file=sys.stderr,
        )

    return SentenceTransformer(model_reference, trust_remote_code=True)
